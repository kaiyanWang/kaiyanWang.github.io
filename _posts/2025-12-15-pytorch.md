...
```
安装完，**关闭当前 ssh 窗口，重新 ssh 登录服务器**，终端前面出现`(base)`代表 conda 生效。
...
第四步：同步代码
使用scp上传代码至服务器，如果需要更改代码，需要在本地更改后重新上传，因此需要同步代码
pycharm中使用存在的conda环境：anaconda/Scripts/conda.exe加载环境，选择py310；vscode中下载python和jupyter插件，选择py310环境
1. PyCharm → Settings → Project → Python Interpreter → Add → On SSH
2. 填入服务器 ssh 信息，解释器路径选你 conda 环境的 python
示例路径：`/home/wky/miniconda3/envs/py310/bin/python`
3. 项目映射：本地代码文件夹 ↔ 远程服务器目录（建议放在 `/home/wky/project/`）
   - 远程路径建议统一：`/home/wky/project/your_exp_name`
4. 配置完成后：
   - 本地修改代码，PyCharm 自动上传到服务器
   - 点运行，代码直接在远程服务器 GPU 上执行，控制台输出回传到本地 PyCharm
登录服务器后创建目录：
```
mkdir -p /home/wky/project    # 存放代码
mkdir -p /home/wky/datasets   # 数据集
mkdir -p /home/wky/output     # 模型权重、日志、实验结果
```
数据集比较大，直接在服务器上 wget 下载，不要从本地上传，太慢！跑深度学习训练动辄几小时，直接在 ssh 终端跑，关闭 ssh 连接程序就被杀掉，因此可以选择tmux。


### featurize云服务器环境配置
把本地的配置文件（[配置文件](https://github.com/kaiyanWang/serverconfig)）上传到云服务器
```bash
