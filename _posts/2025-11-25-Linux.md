...
可以在函数内定义局部变量，作用范围仅在当前函数内；也可以在递归函数中定义局部变量。
```bash
local 变量名=变量值
```
...
[官方文档](https://thrift.apache.org/)
thrift是apache旗下的一个跨语言的RPC框架（Remote Procedure Call，远程过程调用）
...
Linux中，c++编译：
编译：g++ -c main.cpp match_server/*.cpp
链接：g++ *.o -o main -lthrift

如果用到了线程：
链接时：g++ *.o -o main -lthrift -pthread

通过接口生成c++代码：thrift -r --gen cpp ../../thrift/save.thrift

生产者消费者传递消息：消息队列

c++中实现消息队列：锁配合条件变量（条件变量对锁进行封装）

匹配策略：每一秒匹配一次，发现匹配池里有两名玩家分值相差50以内就匹配。（先将所有玩家按分值排序，从第2个玩家开始，如果与前面的分值相差小于50，则匹配并将这两名玩家从匹配池中删除）

匹配策略优化：匹配池中没有匹配上的，允许匹配的分差可以变大，每秒扩大50
...
管道类似于文件重定向，可将前一个命令的 `stdout`（标准输出）重定向到下一个命令的 `stdin`（标准输入）。
1. 管道命令仅处理 `stdout`，会忽略 `stderr`（标准错误输出）。
2. 管道右侧的命令必须能接收 `stdin`。
3. 多个管道命令可以串联使用。

**与文件重定向的区别**

- 文件重定向：左边为命令，右边为文件。
- 管道：左右两边均为命令，左边有 `stdout`，右边有 `stdin`。
...
- 每次 ssh 登陆远程服务器，都会启动一个 bash 命令行。
- 每次 tmux 新开一个 pane，都会启动一个 bash 命令行。
- 所以未来所有新开的环境都会加载我们修改的内容。

#### 常见环境变量
1. **HOME**：用户的家目录。
2. **PATH**：可执行文件（命令）的存储路径，路径之间用 `:` 分隔。当某个可执行文件同时出现在多个路径中时，会选择从左到右第一个路径中的执行。所有存储路径的环境变量，均采用从左到右的优先顺序。
3. **LD_LIBRARY_PATH**：用于指定动态链接库（.so文件）的路径，内容是以冒号分隔的路径列表。
4. **C_INCLUDE_PATH**：C语言的头文件路径，内容是以冒号分隔的路径列表。
5. **CPLUS_INCLUDE_PATH**：C++的头文件路径，内容是以冒号分隔的路径列表。
...
5. **ps aux**：查看所有进程
6. **kill -9 pid**：杀死编号为 `pid` 的进程（[详细信号](https://www.runoob.com/linux/linux-comm-kill.html)）
...
10位：第1位表示类型（`-`为文件，`d`为目录），接下来每3位为一组，分别为当前用户、同组用户、其他用户的权限，r表示可读，w表示可写，x表示可执行

1. **chmod**：修改文件权限
   - `chmod +x xxx`：给 `xxx` 添加可执行权限
   - `chmod -x xxx`：去掉 `xxx` 的可执行权限
   - `chmod 777 xxx`：将 `xxx` 的权限改成rwxrwxrwx（7对应二进制111，即r、w、x都为1）
...
6. **`cut`**：分割一行内容
   - 从 `stdin` 中读入多行数据
   - `echo $PATH | cut -d ':' -f 3,5`：输出 `PATH` 用 `:` 分割后的第 3、5 列数据
   - `echo $PATH | cut -d ':' -f 3-5`：输出 `PATH` 用 `:` 分割后的第 3-5 列数据
...
1. **`more`**：浏览文件内容
   ...
   - `b`：上一页
   - `q`：退出
2. **`less`**：与`more`相似，功能更全
   - 回车：下一行
   - `y`：上一行
   - `Page Down`：下一页
   - `Page Up`：上一页
   - `q`：退出
3. **`head -3 xxx`**：展示xxx的前三行内容，同时支持从stdin读入内容
4. **`tail -3 xxx`**：展示xxx末尾3行内容，同时支持从stdin读入内容

#### 用户相关
1. **`history`**：展示当前用户的历史操作，内容放在`~/bash_history`中

#### 工具
1. **`md5sum`**：计算`md5`哈希值，可以从`stdin`读入内容，也可以在命令行参数中传入文件名列表
...
3. **`pip install xxx --user --upugrade`**：安装python包
# 8 docker
